...
function HomeScreen({ navigation, route }) {
  ...
  const renderItem = ({ item, index }, parallaxProps) => {
    return (
      <TouchableHighlight
        onPress={() => navigation.navigate('Skills', { screen: item.screen })}>
        <View style={screenStyles.item}>
          ...
          <Text style={{ fontSize: wp('100%') * 0.048, color: '#494A56', marginTop: 4}}> {item.title} </Text>
          <View>
    ...
    <View style={styles.MainBackground1}>
      <Header
        ...
        leftComponent=<Avatar
          source={AvatarImages[route.params.avatar].avatar}
          rounded
          size="medium"
      ...
      <Text style={[styles.HomeTextHeading, { marginLeft: '5%' }]}>
        {' '}
        Search{' '}
      </Text>
      <View style={{ alignItems: 'center', height: '10%' }}>
        <SearchBar
          placeholder="What are youj looking for"
